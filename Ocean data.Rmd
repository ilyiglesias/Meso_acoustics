...
This code is preceded by "Analysis 1.Rmd" and followed by "Ocean regrid.Rmd"
...
We can create a simple plot of the location of our acoustic backscatter data from the echo_integration data Lat and Lon data which represents the center point of each 100m horizontal water column (from 175m-675m) of data. Note that this is prior to re-gridding into 25 by 25km grid cells 
...
These data are currently stored in a folder labeled ctd_data. To run the following code,download the CTD files from Dryad and store in a working directory under the same name. (dbo_CTD_HEADER.txt, dbo_CTD_CAST.txt, stations.csv)
```{r load CTD files}
...
ggplot()+
  ...
  theme_classic()
```
# Join vertical CTD data (ctd_cast) to our metadata (ctd_header for 2013:2018)
...
- restrict our geographic range of CTD samples to just be from the core region (defined as 36.45 to 38.33)
```{r}
...
regridded all of the acoustics data to a 25 by 25km grid with CTD overlay
```{r}
ggplot()+
  geom_sf(data=states)+
...
Based on a visual inspection of all CTD profiles, remove the following salinity and density data:
...
Calculated a moving average https://stackoverflow.com/questions/743812/calculating-moving-average 
```{r}
