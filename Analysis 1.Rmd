...
This code is preceded by "Spatial autocorrelation.Rmd" and followed by "Ocean data.Rmd"

# Objective of code
Analysis 1: Are there differences in acoustically detected center of mass depth between years?! (2013-2018)

# Background: 
The following code analyzes whether there were any differences in acoustic backscatter and center of mass depth (m) BETWEEN years in our study. We are interested in inter annual differences AND at varying cell sizes (sensitivity analysis for different regridding sizes)

NOTE: 
- analysis part 1 (this document): Comparing differences in Center of mass depth between years (and diff spatial scales of regridding)
...
Sensitivity analysis: 1km, 5km, 10km, 15km, 20km (from fine-resolution, 100m data originally output)
...
### MINIMUM number of 100m grid cells required for larger grid size
...
ggplot()+
  ...
  geom_point(data = acoustic_regrid, aes(x= lon, y=lat, fill=cm_m), stroke=0, size=7.1, shape=22 )+ #plot regridded data
...
plot depicting the full distribution of MVBS values between years! 
...
ggplot(data = acoustic_regrid, aes(x=cm_m, colour=year))+
  #geom_histogram(aes(y=..density..),color="grey", fill="transparent", binwidth = 50)+ #this is plotting properly for some reason
  geom_density(alpha=0.2, fill="NA", linewidth=1)+ #plot density curves
  scale_color_manual(values = c("2013"="#009E73", "2014"="#56B4E9", "2015"="#E69F00", "2016"="#D55E00", "2017"="#0072B2", "2018"="#CC79A7"))+
  scale_x_reverse(expand=c(0,0))+ #move plot to axis
  scale_y_continuous(expand = c(0,0))+
...
ggsave(paste0("./plots/analysis 1/cm_bw_years_", grid_size, ".pdf"), width=85, units="mm")
...
Although we know there are differences between years, we don't know between WHICH years these differences occur. To compare between groups (years) we thus additionally need to make pairwise comparisons.
...
NOTE: default value for multiple comparisons is "BH" for Bejamini & Hochberg. see p.adjust for more info. Bonferonni conservative option
...
Compact letter display allows us color significant differences in our data
```{r}
...
# References on adding output from multiple comparisons to boxplot
...
ggplot(data=acoustic_regrid, aes(x=year, y=MVBS))+
  ...
  geom_text(data = summary_year, aes(x = year, y = quantile_mvbs, label = cld_mvbs), vjust=-1.8, color="black")+ # label significance cl adjust position
...
ggsave(filename=paste0("./plots/analysis 1/boxplot_kw_mc_mvbs_jitter_", grid_size, "_.pdf"), width=170, height=119, units="mm")
...
# simple box plot of CM values- multiple comparison results from KW 
ggplot(data=acoustic_regrid, aes(x=year, y=cm_m, color=year))+
  stat_boxplot(geom="errorbar", width=0.25)+
...
ggsave(filename=paste0("./plots/analysis 1/boxplot_kw_mc_cm_jitter_", grid_size, "_.pdf"), width=170, height=119, units="mm", bg="transparent")
...
# For sensitivity analysis, re-run for each of the additional grid cell sizes
