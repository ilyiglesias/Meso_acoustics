...
This code is preceded by "Spatial autocorrelation.Rmd" and followed by "Analysis 1.Rmd"
...
Center of Mass (m)- integrated over 175m -- 675m
...
The following uses our echointegrated df echo_integration_all which calculated acoustic metrics for each 100m horizontal water column of water from 150m to 525m. Unlike its original version, echo_integration which selected for only those water columns with less than 10 cells missing per 100m (so at least 75 out of 100 cells per water column,) this version has ALL of the data even if only a few values were available per water column. Because we are regridding to much larger grid cell sizes (1km, 5km, 10km and 20km), we aren't adding the same filter, but want to verify # of smaller cells per new regrid to examine.
...
```
NOTE: for 25km, the total number of 25 by 25km cells is pretty similar: 20, 21, 24, 22, 14, 21
