...
### Background: 
Because our data were collected continuously while underway at sea, and because marine organisms are often grouped in space and time according to ecological drivers, there is some spatial autocorrelation present in our data. This causes problems in terms of analysis, because our samples are not independent (which violates the assumptions of most statistical tests).

In Spatial autocorrelation.Rmd we created correlograms with our mesopelagic depth integrated values (echo_integration df) of 100m horizontal cells (<10 missing vertical cells) and found spatial autocorrelation within our data at approximately ~25km.

From the spatial autocorrelation analysis, a grid size of 25km would be appropriate for acoustic track data (in other words, at 25km, we should no longer have strong spatial autocorrelation in our data and our observations should represent a spatially independent sample). If we re-grid on this scale, the thought is that we would no longer be violating the assumptions of independence in our samples and can proceed with our statistical analysis. However, if we still see a strong spatial pattern in our residuals (both year comparison via kruskall wallis and GAM), may still need to include a spatial component. 
...
A note on geographic coordinate systems: in order to create a spatial grid of a specific distance value (for ex. 5km), I can either keep my inputs in latitude longitude and try to convert our distances into degrees latitude and longitude, OR, convert from Lat Long geographic coordinate system into a projected coordinate system, create the grid of known size (for example UTM ZOne 19N for the core region of California, in meters), and then re-export in degrees latitude longitude (which is what I opted for because it felt more accurate but way more work).

NOTE: although I initially only integrate over water columns 100m horizontal with a maximum of 10 missing cells (ie 65 out of 75m at meso depths), I chose not to do so when we regridded over a larger area because any one area missing backscatter data will likely be compensated by surrounding values and when I looked at the individual echograms created in Read_clean, there didn't appear to be any regions of concern (ie where missing just DSL depths which might otherwise bias results). 

# ADD columns to output df for calculating acoustic metrics
Because we are using the df: output which is our 100m by 5m vertical grid data output from echoview, we need to calculate acoustic metrics from these data (otherwise taking the mean of means from echo_integration 150-525m felt inaccurate). To calculate the MVBS we need the linear mean of Sv (sv) (so convert from Sv (log scale) to sv (linear) and take mean and then re-convert) using the following conversions:
...
```
NOTE: because I am now calculating these acoustic metrics over a much bigger region (instead of individual 100m column of water, 1km, 5km etc) I am no longer going to select for water columns with less than 10 cells missing because this will be averaged over a larger region! To verify this, I looked at the individual echograms from Read clean and I didn't see too many errant values (tho i only calculated CM for those with at least 65 cells (<10 cells missing- the echograms that is) but generally I think it will be okay. (Jarrod also confirmed that this makes sense-not eliminating data from those columns with >10 cells missing when integrating over such a broad area)

For ref: in case we later wanted to assign quality control: In total, there were ~400 records of 30,000 that had less than 65 cells out of 75 vertically (between 150-525m)
# create list of values missing more than 10 cells 
output_remove <- output |> 
                group_by(Year, Day, Interval, Lat_M, Lon_M) |> #group by a specific water column of water, include lat long data
                summarize(n=n(), .groups = "drop") |> 
                filter(n<65)

### Create a grid based on acoustic data extent (specific grid size) 
...
dist= 25000 # this value is in meters (so 5km = 5000m)
...
```
HOORAY!!! we now have nice clean gridlines and cell values for each of our acoustic regridded data which will help facilitate joining to ctd data later
...
- Whereas we were able to apply neat little functions to our output df to calculate additional acoustic metrics, with the rasterize function in the {raster} package, we are only allowed input for ONE variable. The workaround is to calculate intermediary steps as an additional column which we then sum over to regrid at different distances. 

I added a column sv_z: mutate(sv_z=MVBS_linear*Depth_mean) to the output df


Center of mass is thus calculated as: 
sum(z*10^(Sv_z/10))/sum(10^(Sv_z/10)) 
...
- Next, I export both rasters (convert to dataframe) and create a new column for sum(sv_z)/sum(sv) to calculate CM (m)
Also a good check on grid export consistency- should have same centroid values in diff exports!

# Calculate CM for regrid 
Calculate the sum of sv (linear) this is our denominator 
```{r CM : sum of sv}
...
```
# Calculate NASC from vertical 5m binned data for regridded cells- calculate two ways as a check:
...
# Calculate gridded NASC
method 2: 
NASC <- function(Sv_z, cell_H){
 (sum(10^(Sv_z/10))*cell_H)* 4*pi*1852^2
}
NOTE: i also calculated NASC using the formula above but got 156 obs instead of 135, although values were the same, so going to go with the simple addition of all NASC values and not worry about additional calculation 
x_yr<- raster::rasterize(x=echo_yr_sf, y=r, field=echo_yr_sf$sv_cellht, fun=function(x, ...) sum(x)*4*pi*1852^2) #calculate sum of NASC per cell
NOTE: because NASC is an integration I think it is less robust to missing values. Not planning to use this, but it is here for reference! 
...
The following uses our echointegrated df echo_integration_all which calculated acoustic metrics for each 100m horizontal water column of water from 150m to 525m. Unlike its original version, echo_integration which selected for only those water columsn with less than 10 cells missing per 100m (so at least 65 out of 75 cells per water column,) this version has ALL of the data even if only a few values were available per water columns. Because we are regridding to much larger grid cell sizes (1km, 5km, 10km and 20km), I don't think I need to have such a strict value for echo integrating over these larger grid cells, but the output in echo_integration_all may be a little wonky (another reason why I am interseted in looking at it). I am going to regrid these data to the same grid as our other layers whic hwas developed from the original raw output df (which makes sense as that is now consistent with these data)
...
{ggplot(data=acoustics_regrid)+
  ...
  geom_text(aes(x=lon, y=lat, label=echo_count), size=2)+
...
```
NOTE that at this fine resolution there are some cells with only 1 100m horiztonal water column that are defining the whole 25km cell. Will probably have to come up with a way to filter in these situations?! (echo_count) is the col name. Hmmmm


Also note that for all the work it took to use the primary, output df to re-calcualte all of these acoustics metrics, they appear to be the same as the mean of means (eye roll) echo_MVBS
...
# compare amount of data between years?
```{r}
...
TODO: even with 25km, there are some cells that are represented by only a single 100m horizontal cell. I should probbly come up with a way to filter these prior to analysis.



# MANUAL STEP: save output to specific grid size! 
Because I am re-gridding multiple times but don't want to change all of hte code above each time (only have to change the dist= input at the moment), I am saving each regrid output by cell size
...
# clean up objects
```{r}
rm(acoustics_regrid, ext, grid, i, yr)
```
